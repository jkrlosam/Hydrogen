...
double Hydrogen::wave_function_radial(const double r) const{
    ...
    return 2.0/(n*n)*exp(0.5*(lgamma(static_cast<int>(n-l)) - lgamma(static_cast<int>(n+l+1)) ) -r/n)*pow(2.0*r/n,l)* 
...
double Hydrogen::wave_function_theta(const double th) const{
    return sqrt((2.0*l+1.0)/(4.0*M_PI)) * exp(0.5*( lgamma(static_cast<int>(l-abs(m)+1)) - lgamma(static_cast<int>(l+abs(m)+1)) )) *
